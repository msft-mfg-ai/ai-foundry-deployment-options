...
- **Bing Grounding** — either creates a new `Microsoft.Bing/accounts` + `binggrounding` connection on the project, or reuses an existing Bing resource passed as `EXISTING_BING_RESOURCE_ID`. Set `ENABLE_BING_GROUNDING=false` to skip entirely (subs where the Bing RP is suspended and no shared Bing is available). Feeds `BING_CONNECTION_ID`, needed by `tool-web-search`.
