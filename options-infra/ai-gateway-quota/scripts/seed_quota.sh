...
DCE_ENDPOINT=$(azd env get-value DCE_ENDPOINT 2>/dev/null || echo "")
DCR_IMMUTABLE_ID=$(azd env get-value DCR_IMMUTABLE_ID 2>/dev/null || echo "")

if [ -z "$DCE_ENDPOINT" ] || [ -z "$DCR_IMMUTABLE_ID" ]; then
    echo "❌ DCE_ENDPOINT and DCR_IMMUTABLE_ID not set. Run 'azd up' first."
    exit 1
fi
...
[
  ...
  {"TimeGenerated": "$NOW", "CallerAzp": "$TEAM_GAMMA_APP_ID", "CallerName": "Team Gamma", "MonthlyTokenBudget": 100}
]
EOF
)

RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
    "${DCE_ENDPOINT}/dataCollectionRules/${DCR_IMMUTABLE_ID}/streams/Custom-Json-CALLER_QUOTA_CL?api-version=2023-01-01" \
...
if [ "$HTTP_CODE" = "204" ] || [ "$HTTP_CODE" = "200" ]; then
    ...
    echo "   Team Gamma:  100 tokens/month (test)"
else
