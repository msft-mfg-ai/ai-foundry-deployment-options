 ...
 * `wss://` backend URL via a custom `onHandshake` policy
...
// deployment name (first route wins) so multiple instances serving the same
// realtime deployment collapse to a single backend (no LB on WebSocket
// connections).
