...
# Configuration
PROCESSING_DELAY_SECONDS = int(os.environ.get("PROCESSING_DELAY_SECONDS", "180"))  # 3 minutes default
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    setup_telemetry()
    logger.info(f"File Processor Service starting. Delay configured: {PROCESSING_DELAY_SECONDS}s")
    yield
    # Shutdown
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            "health": "/health",
            "process_files": "/process-files (POST)"
        }
    }
