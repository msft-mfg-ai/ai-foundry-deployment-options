...
class AgentManager:
        ...
        with tracer.start_as_current_span("agent_manager_initialize"):
            try:
                ...
                if settings.AZURE_AI_PROJECT_CONNECTION_STRING:
                    ...
                    self.large_context_plugin = LargeContextAgentPlugin(
                        self.file_processor_plugin
                    )
    ...
    async def _setup_agents(self) -> None:
        ...
        logger.info(
            f"AzureAIAgentSettings - endpoint: {self.ai_agent_settings.endpoint}"
        )
        logger.info(
            f"AzureAIAgentSettings - model: {self.ai_agent_settings.model_deployment_name}"
        )
    ...
    async def _setup_large_context_agent(self, existing_agents: dict) -> None:
        ...
        logger.info(
            f"Large Context Agent ready - ID: {agent_definition.id}, Name: {agent_definition.name}"
        )
    ...
    async def _setup_master_agent(self, existing_agents: dict) -> None:
        ...
        logger.info(
            f"Master Agent ready - ID: {agent_definition.id}, Name: {agent_definition.name}"
        )
        ...
        with tracer.start_as_current_span("large_context_agent_invoke") as span:
            ...
            try:
                if self.large_context_agent:
                    ...
                    logger.info(
                        "Created new Large Context Agent thread for this invocation"
                    )
            ...
            try:
                if self.master_agent:
                    ...
                    if self.master_thread is None:
                        self.master_thread = AzureAIAgentThread(
                            client=self.agent_client
                        )
                    ...
                    logger.info(
                        f"Invoking Master Agent with message: {message[:100]}..."
                    )
                    ...
                    async for agent_response in self.master_agent.invoke(
                        ...
                        for item in agent_response.items or []:
                            if isinstance(item, TextContent):
                                response_text = item.text
                                logger.info(
                                    f"Got text response: {response_text[:100]}..."
                                )
                            elif isinstance(item, FunctionCallContent):
                                plugins_invoked.append(item.name)
                    ...
                    logger.info(
                        f"Master Agent completed. Processed {response_count} responses."
                    )
                    ...
                    # Mock mode response
                    response_text, plugins_invoked = await self._generate_mock_response(
                        message
                    )
